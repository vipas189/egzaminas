...
from models.form.test_form import TestForm, TestQuestionForm
...
def test_routes(app):
    ...
    def list_tests():
        ...
        return render_template(
            "tests/index.html", tests=tests, instructor_model=Instructor
        )
    ...
    def instructor_tests(instructor_id):
        ...
        return render_template(
            "tests/instructor_tests.html",
            instructor=instructor,
            tests=tests,
            modules=Modules,
        )  # Perduoti Modules modelį į šabloną
    ...
    def edit_test_questions(test_id):
        ...
        if request.method == "POST":
            ...
            db.session.commit()
            return jsonify(
                {"status": "success", "message": "Klausimai išsaugoti sėkmingai"}
            )

        questions = TestQuestion.query.filter_by(test_id=test.id).all()
        return render_template(
            "tests/edit_questions.html", test=test, questions=questions
        )
    ...
    def student_tests(student_id):
        ...
        return render_template(
            "tests/student_tests.html", student=student, available_tests=available_tests
        )

    @app.route(
        "/students/<int:student_id>/tests/<int:test_id>/start", methods=["GET", "POST"]
    )
    def start_test(student_id, test_id):
        ...
        attempt = TestAttempt(
            test_id=test.id, student_id=student.id, start_time=datetime.now()
        )
    ...
    @app.route(
        "/students/<int:student_id>/tests/<int:test_id>/attempt/<int:attempt_id>",
        methods=["GET", "POST"],
    )
                    ...
                    if question.question_type == "multiple_choice":
                        ...
                        if selected_option_id:
                            ...
                            answer = TestAnswer(
                                ...
                                points_earned=(
                                    question.points if option.is_correct else 0
                                ),
                            )
    ...
    def grade_test(attempt):
        ...
        student = Users.query.get(attempt.student_id)
        ...
        earned_points = sum(
            a.points_earned or 0 for a in answers if a.points_earned is not None
        )
        ...
        if attempt.passed and test.weight > 0:
            ...
            existing_grade = StudentGrade.query.filter_by(
                student_id=student.id, module_id=test.module_id, test_id=test.id
            ).first()
    ...
    @app.route(
        "/students/<int:student_id>/tests/<int:test_id>/attempt/<int:attempt_id>/results"
    )
        ...
        for question in questions:
            ...
            if answer:
                if question.question_type in ["multiple_choice", "true_false"]:
                    selected_option = (
                        TestQuestionOption.query.get(answer.selected_option_id)
                        if answer.selected_option_id
                        else None
                    )
    ...
    @app.route(
        "/instructors/<int:instructor_id>/tests/<int:test_id>/attempt/<int:attempt_id>/review",
        methods=["GET", "POST"],
    )
    def review_student_attempt(instructor_id, test_id, attempt_id):
        ...
        for question in questions:
            ...
            if answer:
                if question.question_type in ["multiple_choice", "true_false"]:
                    ...
                    correct_options = TestQuestionOption.query.filter_by(
        ...
        return render_template(
            "tests/review_student_attempt.html",
            instructor=instructor,
            test=test,
            attempt=attempt,
            student=student,
