...
from models.form.group_form import GroupFormationForm, GroupForm
...
def group_routes(app):
    ...
    def create_group():
        ...
        if form.validate_on_submit():
            ...
            group = StudentGroup(name=group_name, program_id=form.program_id.data)
    ...
    def form_groups():
        ...
        if form.validate_on_submit():
            ...
            students = Users.query.filter_by(program_id=program_id).all()
            ...
            for i in range(group_count):
                ...
                group = StudentGroup(name=group_name, program_id=program_id)
                db.session.add(group)
            ...
            db.session.commit()
            flash(
                f"Sukurtos {group_count} grupės ir paskirstyti {total_students} studentai",
                "success",
            )
            return redirect(url_for("list_groups"))
    ...
    def manage_group_students(id):
        ...
        if request.method == "POST":
            ...
            students = Users.query.filter(Users.id.in_(student_ids)).all()
        ...
        program_students = Users.query.filter_by(program_id=group.program_id).all()

        return render_template(
            "groups/manage_students.html",
            group=group,
